...
public class OneDrawable {
	...
	public static Drawable createBgDrawableWiehWhiteMode(@NonNull Context context, @DrawableRes int res, @FloatRange(from = 0.0f, to = 1.0f) float alpha) {
		return createBgDrawable(context, res, PressedMode.WHITE, alpha);
	}
    ...
    private static Drawable getPressedStateDrawable(Context context, @PressedMode.Mode int mode, @FloatRange(from = 0.0f, to = 1.0f) float alpha, @NonNull Drawable pressed) {
        ...
        switch (mode) {
            ...
            case PressedMode.DARK:
                ...
                break;
			case PressedMode.WHITE:
				pressed.setColorFilter(alphaColor(Color.WHITE, convertAlphaToInt(alpha)), PorterDuff.Mode.SRC_ATOP);
				break;
			default:
                pressed.setAlpha(convertAlphaToInt(alpha));
        }
